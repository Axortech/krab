        ...
        Node::Element(v_el) => {
            if let Some(real_node) = real_node_opt {
                ...
                if match_found {
                    ...
                    console::warn_1(
                        &format!(
                            "Hydration mismatch at index {}: expected <{}>, replacing.",
                            index, v_el.tag
                        )
                        .into(),
                    );
                ...
                console::warn_1(
                    &format!("Hydration mismatch: expected <{}>, appending.", v_el.tag).into(),
                );
                if let Some(new_node) = create_dom_node(v_node) {
        ...
        Node::Dynamic(f) => {
            ...
            create_effect(move || {
                ...
                let Some(new_dom_node) = create_dom_node(&new_v_node) else {
                    console::error_1(
                        &"{\"scope\":\"dynamic\",\"detail\":\"failed to create replacement node\"}"
                            .into(),
                    );
                    return;
                };
